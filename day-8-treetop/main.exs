defmodule Treehouse do
  ...
  defp cast(trees, height) do
    Enum.reduce_while(trees, 0, fn (c, acc) -> if c >= height, do: {:halt, acc + 1}, else: {:cont, acc + 1} end)
  end
  ...
  def find_visible_trees(filename) do
    grid = File.stream!(filename) |> Enum.reduce([], fn (line, acc) ->
      acc ++ [line |> String.trim() |> String.split("", trim: true) |> Enum.map(fn s ->
        {int, _rem} = Integer.parse(s)
        int
      end)]
    end)

    grid |> Enum.with_index |> Enum.reduce(0, fn ({row, y}, acc) ->
      acc + (row |> Enum.with_index |> Enum.reduce(0, fn ({_cell, x}, a) -> if is_visible(grid, x, y), do: a + 1, else: a end))
    end)
  end
  ...
  def find_most_scenic_tree(filename) do
    ...
    grid |> Enum.with_index |> Enum.reduce(0, fn ({row, y}, acc) ->
      Enum.max([acc, row |> Enum.with_index |> Enum.reduce(0, fn ({_cell, x}, a) ->
